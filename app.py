...
#train_data (전력 수요)
supply = pd.read_csv("C:\Users\one\Desktop\2차 인사이콘\한국전력거래소_5분단위 전력수급현황_20220407.csv", encoding='cp949')
supply["기준일시"] = pd.to_datetime(supply["기준일시"])
supply = supply[['기준일시', '현재수요(MW)']]

#test_data(전력 수요)
supply_want = pd.read_csv("./전력.csv")
...
#test_data(날씨)
weather_want = pd.read_csv("./기상.csv",encoding = "ANSI")
